#' Scrapes SIM's ICD-10 data from  Minas Gerais cities
#'
...
#'
#' 
#' @usage sim_obt10_mun_mg(linha = "Município", coluna = "Não ativa",
#'   conteudo = 2, periodo = "last", municipio = "all", capital = "all",
#'   cir = "all", macrorregiao_de_saude = "all", divisao_administ_estadual = "all", microrregiao_ibge = "all",
#'   ride = "all", grande_grupo_cid10 = "all", grupo_cid10 = "all", categoria_cid10 = "all",
#'   faixa_etaria = "all", faixa_etaria_ops = "all", faixa_etaria_det = "all", faixa_etaria_menor1a = "all",
#'   sexo = "all", cor_raca = "all", escolaridade = "all", estado_civil = "all", local_ocorrencia = "all",
#'   acid_trabalho = "all")
#' 
#' @param prelimares A boolean indicating if the preliminary data base will be considered or not. Defaults to FALSE.
...
#' @param conteudo A character of length = 1 (1 for Óbitos_p/Residênc and 2 for Óbitos_p/Ocorrênc). Defaults to 2
...
#' @param divisao_administ_estadual "all" or a numeric vector with the state administrative division's codes to filter the data. Defaults to "all".
...
#' @param grande_grupo_cid10 "all" or a character vector with the ICD-10 group (written in the same way) or the number corresponding to the order of the option in the online layout to filter the data. Defaults to "all".
...
#' @param acid_trabalho "all" or a character vector with "Sim", "Não" or "Ignorado"  to filter the data. Defaults to "all".
...
#' @examples
#' \dontrun{
#' ## Requesting data related to male deaths caused by transportation accidents of all cities in Minas Gerais 
#' sim_obt10_mun_mg(grande_grupo_cid10 = "V01-V99 Acidentes de transporte", sexo="Masc")
#' }
#'
...
sim_obt10_mun_mg <- function(preliminares = FALSE, linha = "Munic\u00edpio", coluna = "N\u00e3o ativa", conteudo = 2, periodo = "last", municipio = "all",
                             regiao_de_saudecir = "all", macrorregiao_de_saude = "all", divisao_administ_estadual = "all", microrregiao_ibge = "all",
                             ride = "all", grande_grupo_cid10 = "all", grupo_cid10 = "all", categoria_cid10 = "all", faixa_etaria = "all", 
                             faixa_etaria_ops = "all", faixa_etaria_det = "all", faixa_etaria_menor1a = "all", sexo = "all", 
                             cor_raca = "all", escolaridade = "all", estado_civil = "all", local_ocorrencia = "all", acid_trabalho = "all") {
  
  
  if (preliminares){
    url <- "http://tabnet.datasus.gov.br/cgi/deftohtm.exe?sim/cnv/pext10mg.def"
  }else{
    url <- "http://tabnet.datasus.gov.br/cgi/deftohtm.exe?sim/cnv/ext10mg.def"
  }
  
  #verifies if the preliminary data base is available
  if(preliminares){
    p <- curlGetHeaders(url)
    status <- attr(p, "status")
    if(status == 503){
      stop("A página dos dados preliminares do DATASUS está fora do ar")
    } else if(status == 200){
      p <- read_html(url)
      dados <- p |> html_elements(xpath = "//h2") |> html_text2()
      if(dados[2] == 'Nenhum dado disponível'){
        stop("Os dados preliminares do DATASUS não estão disponíveis")
      } else{
        opcoes <- p |> html_elements(xpath = "//select[@name = 'Arquivos']") |> html_text2()
        opcoes <- strsplit(opcoes, " ")
        if(!(as.character(paste0(periodo, '\r')) %in% opcoes[[1]])){ #verifica se existe o ano desejado
    ...
    status <- attr(p, "status")
    if(status == 503){
      stop("A página dos dados consolidados do DATASUS está fora do ar")
    } 
  ...
  divisao_administ_estadual.df <- suppressWarnings(data.frame(id = page |> rvest::html_nodes("#S4 option") |> rvest::html_text() |> readr::parse_number(),
  ...
  faixa_etaria.df$id[1] <- faixa_etaria_ops.df$id[1] <- faixa_etaria_det.df$id[1]  <- faixa_etaria_menor1a.df$id[1] <- "all"
  sexo.df$id[1] <- cor_raca.df$id[1] <- escolaridade.df$id[1] <- estado_civil.df$id[1] <- local_ocorrencia.df$id[1] <- acid_trabalho.df$id[1] <- "all"
  ...
  if (any(divisao_administ_estadual != "all")) {
    ...
    if (!(all(divisao_administ_estadual %in% divisao_administ_estadual$id))) stop("Some element in 'divisao_administ_estadual' argument is wrong")
  ...
  if (any(grupo_cid10 != "all")) {
    ...
    grupo_cid10 <- as.character(as.numeric(grupo_cid10))
  ...
  if (any(categoria_cid10 != "all")) {
    ...
    if (!(all(categoria_cid10 %in% categoria_cid10.df$id))) stop("The element in 'categoria_cid10' argument is wrong")
  ...
  form_pesqmes2 <- "pesqmes1=Digite+o+texto+e+ache+f%E1cil"
  ...
  form_pesqmes3 <- "pesqmes4=Digite+o+texto+e+ache+f%E1cil"
  ...
  #divisão administrativa estadual
  ...
  form_grande_grupo_cid10 <- paste0("SGrande_Grupo_CID10=", form_grande_grupo_cid10$value, collapse = "&")
  ...
  form_grupo_cid10 <- paste0("SGrupo_CID10=", form_grupo_cid10$value, collapse = "&")
  ...
  form_categoria_cid10 <- paste0("SCategoria_CID10=", form_categoria_cid10$value, collapse = "&")
  ...
  form_acid_trabalho <- paste0("SAcid%2E_Trabalho=", form_acid_trabalho$value, collapse = "&")
  
  
  form_data <- paste(form_linha, form_coluna, form_conteudo, form_periodo, 
                     form_pesqmes1, form_municipio, form_pesqmes2, form_cir, form_pesqmes3, form_macrorregiao_de_saude,
                     form_pesqmes4, form_divisao_administ_estadual, form_pesqmes5, form_microrregiao_ibge, form_ride,
                     form_grande_grupo_cid10, form_pesqmes8, form_grupo_cid10, form_pesqmes9, form_categoria_cid10,
                     form_pesqmes10, form_faixa_etaria, form_pesqmes11, form_faixa_etaria_ops, form_faixa_etaria_det,
                     form_faixa_etaria_menor1a, form_sexo, form_cor_raca, form_escolaridade, form_estado_civil, 
                     form_local_ocorrencia, form_acid_trabalho,
