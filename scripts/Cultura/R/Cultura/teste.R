...
dados_iepha <- as_tibble(readxl::read_excel("iepha_2020.xlsx", sheet =1))
...
dados_iepha <- dados_iepha %>% select(c(2, 22, 21, 3, 4, 5, 34, 25, 33))   
...
#' pdf para excel 
dados_iepha <- dados_iepha[-c(1:6), ]   
dados_iepha <- subset(dados_iepha, municipio != 'B')


#' Atualiza a coluna de nomes dos municípios para conter somente os nomes e cria uma nova coluna
#' que conterá os números
dados_iepha <- dados_iepha %>% mutate(municipio = tolower(dados_iepha$municipio))
...
indicadores <- indicadores %>% mutate(C_PCL = apply(indicadores[, c('pontuacao_politica_cultural',
                                                                    ...
                                                                    'pontuacao_educacao_e_difusao')], 1, 
