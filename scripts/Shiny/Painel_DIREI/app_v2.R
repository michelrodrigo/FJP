...
layout_app <- read_excel(tf, 7L)
grafico1 <- read_excel(tf, 8L)
grafico2 <- read_excel(tf, 9L)
options(OutDec= ".") 
...
server <- function(input, output) {
    ...
    output$graficos <- renderUI({
      plot_output_list <- lapply(seq(10), function(l){
        if(layout_app$Colunas[l] > 0){
          fluidRow(
            lapply(seq(layout_app$Colunas[l]), function(i) {
              plotname <- paste("plot", l, i, sep="")
              box(
                status = "primary",
                solidHeader = FALSE,
                width = 12/layout_app$Colunas[l],
    ...
    for (l in 1:1) {
      for(i in 1:layout_app$Colunas[l]){
        ...
        local({
          my_i <- i
          plotname <- paste("plot", l, i, sep="")
          grafico <- layout_app[layout_app$Linha == l & layout_app$Coluna==i, ]$grafico[1]
          ...
          dados1 <- dados[dados$ano %in% grafico$ano, c("ano", grafico$x[1], grafico$y[1])]
          ifelse(grafico$tipos_rotulos_eixo_x[1]== "mes", cat <- meses_sigla, ifelse(grafico$tipos_rotulos_eixo_x[1]== "ano", cat <- grafico$ano))
          ...
          output[[plotname]] <- renderHighchart({
            ...
            h <- highchart() |>
              hc_chart(type = grafico$tipo_grafico[1]) |>
              hc_title(text = paste(grafico$titulo[1]))|>
              hc_subtitle(text = grafico$sub_titulo[1]) |>
              hc_plotOptions(bar=list(dataLabels=list(enabled=TRUE,
                                                      format='R$ {point.y:,.2f}',
                                                      align = 'left'),
                                      pointWidth = 30)) |>
              hc_tooltip(headerFormat = '') |>
              hc_yAxis(labels = list(enabled=grafico$exibir_rotulo_eixo_y), gridLineWidth = 0, title = list(text = grafico$titulo_eixo_y[1])) |>
              hc_xAxis(labels = list(enabled=grafico$exibir_rotulo_eixo_x), min = , max=11, title = list(text = ifelse(grafico$titulo_eixo_x[1]=="NA", NA, grafico$titulo_eixo_x[1])),
                       categories = cat) |>
            ...
            if(grafico$tipo_grafico[1] == "column"){
              h <- h |> hc_plotOptions(column=list(dataLabels=list(enabled=grafico$exibir_rotulos_serie[1],
                                                         format='{point.y:.2f}',
                                                         align = 'center' ,rotation = grafico$rotacao_rotulos[1]
            ...
            for (k in 1:length(anos)) {
              dados2 <- data.frame(x= dados1[dados1$ano == grafico$ano[k], grafico$x[1]],
                                   y= dados1[dados1$ano == grafico$ano[k], grafico$y[1]])
              colnames(dados2) <- c("x", "y")
              h <- h |>
                hc_add_series(data = dados2, name = grafico$nome[k])
            }
