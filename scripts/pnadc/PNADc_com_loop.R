...
for(ano in vetor_anos){
  ...
  for(trimestre in vetor_trimestres){
    ...
    dadospnadc <- get_pnadc (year = ano, quarter = trimestre, vars=c("UF","V2007","VD4001","VD4002", "VD4003", "V2009"))
...
oc.desc_mg <- as.data.frame(svytotal(~interaction (VD4001, VD4002), subset(dadospnadc, UF == "Minas Gerais"), na.rm = T))
oc.desc_mg <- mutate(oc.desc_mg, variavel = row.names(oc.desc_mg))
oc.desc_mg <- pivot_wider(oc.desc_mg[,-2], names_from = variavel, values_from = total)
oc.desc_mg <- oc.desc_mg %>% mutate(ano = ano) %>% mutate(trimestre = trimestre)
OC.DESOC.MG_tab <- rbind (OC.DESOC.MG_tab, oc.desc_mg) 
OC.DESOC.MG_tab <- OC.DESOC.MG_tab %>% select(ano, trimestre, everything())

TX DESOC = DESOC. / PEA (ocupado + DESOC)